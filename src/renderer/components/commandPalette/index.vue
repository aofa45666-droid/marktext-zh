      ...
      <div slot="title" class="search-wrapper">
        ...
        <transition name="fade" v-else-if="availableCommands.length">
          <ul class="commands">
            ...
            >
              <span class="title" :title="item.title">{{tCmdDesc(item.id, item.description)}}</span>
              <span class="shortcut">
                <span
                  class="shortcut"
...
import zhCNDict from '../../commands/descriptions-zh-CN'
...
export default {
  ...
  data () {
    ...
    this.defaultPlaceholderText = this.$t ? this.$t('Type a command to execute') : 'Type a command to execute'
    return {
  ...
  methods: {
    tCmdDesc (id, fallback) {
      if (this.currentLanguage === 'zh-CN') {
        return zhCNDict[id] || fallback
      }
      return fallback
    },
    handleShow (command) {
