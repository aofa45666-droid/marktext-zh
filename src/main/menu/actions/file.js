...
const isChineseLang = () => {
  try {
    const pref = JSON.parse(fs.readFileSync(path.join(app.getPath('userData'), 'preferences.json'), 'utf8'))
    return pref.language === 'zh-CN'
...
const showUnsavedFilesMessage = async (win, files) => {
  ...
  const message = isZh
    ? `是否保存对 ${fileCount} 个文件的更改？\n\n${files.map(f => f.filename).join('\n')}`
    : `Do you want to save the changes you made to ${fileCount} ${fileCount === 1 ? 'file' : 'files'}?\n\n${files.map(f => f.filename).join('\n')}`
  const detail = isZh ? '如果不保存，更改将丢失。' : 'Your changes will be lost if you don\'t save them.'
...
ipcMain.on('mt::rename', async (e, { id, pathname, newPathname }) => {
  ...
  if (!await exists(newPathname)) {
    ...
    const { response } = await dialog.showMessageBox(win, {
      type: 'warning',
      buttons: isZh ? ['替换', '取消'] : ['Replace', 'Cancel'],
      defaultId: 1,
      message: isZh
        ? `文件 "${path.basename(newPathname)}" 已存在。是否替换？`
        : `The file "${path.basename(newPathname)}" already exists. Do you want to replace it?`,
      cancelId: 1,
      noLink: true
    })
