...
class AppMenu {
  ...
  constructor (preferences, keybindings, userDataPath) {
    ...
    this._localeData = loadLocale(this._preferences.getItem('language'))
  ...
  _buildEditorMenu (recentUsedDocuments = null) {
    ...
    const menuTemplate = configureMenu(this._keybindings, this._preferences, recentUsedDocuments, this._localeData)
  ...
  _buildSettingMenu () {
    if (isOsx) {
      const menuTemplate = configSettingMenu(this._keybindings, this._localeData)
      const menu = Menu.buildFromTemplate(menuTemplate)
  ...
  _listenForIpcMain () {
    ...
    ipcMain.on('broadcast-preferences-changed', prefs => {
      ...
      // Reload locale when language changes and rebuild menus
      if (prefs.language !== undefined) {
        this._localeData = loadLocale(prefs.language)
        this.updateAppMenu()
      }
    })
  }
